...
const authCtrl = {
    register: async (req, res) => {
        try {
            ...
            if (password.length < 6)
                return res.status(400).json({ msg: "This password must at least 6 character" })
    ...
    login: async (req, res) => {
        try {
            ...
            if (!user) return res.status(400).json({ msg: "This email not exit" })
    ...
    generateAccessToken: async (req, res) => {
        try {
            ...
            jwt.verify(rf_token, process.env.REFRESH_TOKEN_SECRET, async (err, result) => {
                ...
                if (!user) return res.status(400).json({ msg: "Doesn't exist" })
